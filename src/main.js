...
function saveGameState() {
  if (isGameOver) {
    // Don't save if game is over, allow new game on next open
...
function updateBoard() {
  for (let i = 0; i < MAX_ATTEMPTS; i++) {
    ...
    for (let j = 0; j < WORD_LENGTH; j++) {
      ...
      const letter =
        i === currentRowIndex && currentGuess[j] ? currentGuess[j] : "";
...
function getFeedback(guessArray) {
  ...
  for (let i = 0; i < WORD_LENGTH; i++) {
    if (feedback[i] === null) {
      ...
      if (
        targetArray.includes(guessArray[i]) &&
        targetLetterCounts[guessArray[i]] > 0
      ) {
...
function checkWinOrLoss(guessStr, withSave) {
  if (guessStr === targetWord) {
    ...
    if (withSave)
      showMessage(`😅 Game Over! Word was: ${targetWord}`, "error", 0); // Persistent
    isGameOver = true;
  }
